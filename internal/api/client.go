...
// ValidateBaseURL checks that the API URL is safe to use.
// HTTPS required for remote URLs. HTTP only allowed for localhost/Docker dev.
func ValidateBaseURL(baseURL string) error {
	if baseURL == "" {
		return fmt.Errorf("API URL is required")
	}

	// Allow HTTP only for local development (localhost, 127.0.0.1, Docker host)
	...
	// Exact matches without port/path
	if baseURL == "http://localhost" || baseURL == "http://127.0.0.1" || baseURL == "http://[::1]" {
		return nil
	}
...
func (c *Client) PollPairingStatus(deviceCode string) (string, error) {
	...
	// 429 = rate limited — return "pending" so the caller waits and retries
